defmodule IslandsInterfaceWeb.GameChannel do
  ...
  def handle_in("add_player", player, socket) do
    case Game.add_player(via(socket.topic), player) do
      :ok ->
        broadcast! socket, "player_added", %{message: "New plater just joined: " <> player}
        {:noreply, socket}
      {:error, reason} ->
        error_reply(reason, socket)
      :error ->
  ...
  def handle_in("position_islands", payload, socket) do
    %{"player" => player, "island" => island, "row" => row, "col" => col} = payload
    player = String.to_existing_atom(player)
    island = String.to_existing_atom(island)
    case Game.position(via(socket.topic), player, island, row, col) do
      :ok -> {:reply, :ok,    socket}
  ...
  def handle_in("guess_coordinate", payload, socket) do
    ...
    player = String.to_existing_atom(player)
    case Game.guess_island(via(socket.topic), player, row, col) do
      {:hit, island, win} ->
      ...
      {:miss, island, win} ->
        ...
        broadcast! socket, "player_guessed_coordinate", %{player: player, row: row, col: col, result: result}
        {:noreply, socket}
      :error -> {:reply, %{player: player, reason: "Not your turn"}, socket}
  ...
  defp error_reply(reason, socket), do: {:reply, {:error, %{reason: inspect(reason)}}, socket}
